...
function getCirclesProperties(radiuses) {
  const result = radiuses.map(function(calcul){
    let obj1 = {radius: calcul};

    obj1.circumference = (2 * Math.PI * calcul).toFixed(3)
    obj1.surface = (Math.PI * calcul * calcul).toFixed(3)
    return obj1
  })
  return(result)
}
